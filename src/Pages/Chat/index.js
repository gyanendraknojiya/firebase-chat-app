...
const Chat = () => {
  ...
  const scroll_to = (div) => {
    if (div.scrollTop < div.scrollHeight - div.clientHeight) div.scrollTop += div.scrollHeight;
  };
  useEffect(() => {
  ...
  return (
    <div className="d-flex vh-100 vw-100 justify-content-center align-items-center">
      <div style={{ maxWidth: 600, width: '90%', height: '90vh' }} className="rounded shadow p-2 text-white bg-dark">
        <div style={{ height: 150 }}>
          <div className="d-flex">
            <span className="ms-auto btn btn-danger btn-sm" onClick={handleLogout}>
              Logout
            </span>
          </div>
        ...
        <div id="messages" className="bg-white rounded " style={{ height: 'calc(100% - 200px)', overflowY: 'auto' }}>
          {messages.length ? (
              ...
              <div className="d-flex" key={i}>
                ...
                >
                  <div style={{ fontSize: 11 }}>
                    ...
                    <span className="float-end text-muted">{new Date(message.createdAt).toUTCString()}</span>
                  </div>
                  {message.message}
                </div>
              </div>
